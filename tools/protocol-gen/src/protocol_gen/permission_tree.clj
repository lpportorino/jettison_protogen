(ns protocol-gen.permission-tree
  "The NESTED permission mirror: one static tree per group, in Rust, for a
   ...
   WHY A TREE RATHER THAN THE FLAT TRANSCRIPT. `protocol-gen.mirror` emits the
   permission TRANSCRIPT, a FLAT map — group, message, field, number,
   provenance — and a byte-level scanner cannot walk it. A scanner holds a
   position in a message and a tag it has just read; what it needs at that
   instant is the node for THAT tag under the node it is standing on, and a
   flat map has no `under`. The two artefacts also carry different facts: the
   transcript carries NUMBER PROVENANCE and DIRECTION, which a scanner does not
   consult, and carries no permission axis at all. Neither is the other under
   another name, and neither replaces the other here.

   THE WORD MIRROR IS THIS TREE'S ALONE. Two artefacts under one name leave a
   reader sent to `the permission mirror` unable to tell which is meant, so the
   flat one is the TRANSCRIPT; `protocol-gen.mirror`'s docstring carries why
   its namespace spells the word this one keeps.
...
(def banner
  ...
  (str "// GENERATED by protocol-gen — DO NOT EDIT.\n"
       ...
       "// `.proto`, its Rust access module and the permission transcript.\n"
       "//\n"
