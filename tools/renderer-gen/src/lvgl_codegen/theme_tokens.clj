...
(def fields
  ...
  [{:sem-key :surface-1
   ...
   {:sem-key :surface-2
    :c-macro "THEME_SURFACE2"
    :kind :color
    :doc "elevated fill (buttonmatrix items)"}
   ...
   {:sem-key :accent-bg
    :c-macro "THEME_ACCENT"
    :kind :color
    :doc "accent fill — the asgard family's stock-parent color_primary"}
...
(defn generate-header!
  "Write the generated C header (renderer/generated/theme_tokens.h) from the
   EDN home. Run via the `manifests` freshness lane (renderer.mk), which
   re-emits the header on every battery run and FATALs on drift."
  [home-path out-path]
...
(defn -main
  "Read the token home, emit the theme_tokens.h header text to `--output`,
   print the destination. `--output` is mandatory — this tool never guesses
   a destination."
  [& args]
  (let [{:keys [tokens output]} (parse-args args)]
    ...
    (println (str "theme_tokens.h: emitted from " tokens " -> " output))))
