...
(defn brief-md
  ...
  (str
   ...
   "Running this review is required before any push that modifies "
   "elements — in this repo and in every consumer that pins it "
   "(`CLAUDE.md` §\"Consuming the UI standard\"). §5 is why it cannot be "
   "one repo's habit: a quality rule that lives only in a consumer is a "
   "defect live in every other consumer that has not yet tripped over "
   "it.\n\n"
   ...
   "The corollary about readability: §0 gives that lane a three-way "
   "verdict whose uncertain band may only be adjudicated by a classifier "
   "validated on a held-out labelled set. You have not been validated as "
   "one. Report a legibility doubt as a finding for a human; never report "
   "it as a readability pass or fail.\n\n"
   ...
   "- **Extended click areas.** The rules measure `:coords`; the pointer "
   "is tested against a grown box (§2.4), so the true hazard boundary can "
   "be larger than anything visible.\n"
