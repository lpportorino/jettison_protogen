...
## ts-validated-repro / ts-validated-repro-test: the committed TypeScript is
## exactly what the PINNED protoc-gen-es produces
# Dockerfile.base pins @bufbuild/protoc-gen-es because it REWRITES committed
# source and stamps its own version into the first line of every file it emits.
# The pin is the promise; tools/ts_validated_repro.sh is the proof — it re-runs
# generate-protos.sh's typescript-validated leg into a throwaway directory and
# refuses on any byte difference against output/typescript-validated, plus a PIN
# clause byte-identity structurally cannot cover (a tree generated by a stale
# image and compared against that same stale image agrees with itself and
# disagrees with Dockerfile.base).
#
# THE SPLIT IS THE POINT, AND IT IS WHY ONLY THE CANARY IS ON THIS LINE.
#
#   ts-validated-repro       NOT in `lint-lanes`, on purpose — the same call
#     Makefile's go-leg-repro and orphan-scan make, and for their reason. It
#     needs docker plus the multi-gigabyte generator image, while `lint` is
#     invoked BARE on a plain host (and inside tools/uber.sh, whose image ships
#     no docker CLI). Folding it in would hard-fail every push from a machine
#     without them, or — far worse — have to skip, and a skip here is a green
#     tick over zero coverage. It is a host-only, on-demand check, and it lives
#     here so that it and its canary are discoverable and runnable.
#
#   ts-validated-repro-test  IN `lint-lanes`, because it is HERMETIC. Its cases
#     are synthetic trees under a mktemp directory driving the comparison
#     clauses directly, so it needs neither docker nor an image nor the network
#     — the same reason wasm-provenance-test above rides `lint` while the gate
#     it proves is armed elsewhere. It also drives the two LIVE-tree readers as
#     controls (the pin out of Dockerfile.base, the payload out of
#     generate-protos.sh), which is what catches the shape that would otherwise
#     let the check silently generate nothing.
#
# SO A GREEN CANARY IS NOT A GREEN CHECK, and reading it as one is the mistake
# this comment exists to prevent: the canary proves the verdict logic can fail
# and that each red is attributable to its own clause; whether the committed
# bytes still reproduce is answered only by running ts-validated-repro.
#
# ITS CLAUSES ARE MUTATION-PROVEN TO FIRE ALONE. Disabling the content
# comparison reds exactly the two CONTENT assertions; disabling the pin
# comparison reds exactly the two PIN ones; dropping the missing-file collection
# reds exactly the two ABSENT ones; and turning every ERROR return into 0 reds
# exactly the four precondition assertions — each leaving every neighbour green,
# and each an assertion FAILURE rather than a harness error.
#
# NEITHER IS IN A WORKFLOW. The canary is hook-only, joining the family
# `.claude/rules/lint-gates.md` already names as having no authoritative half;
# the check cannot be wired into build-and-release.yml, where the comparison
# would be VACUOUS after `make generate` has just written those exact bytes with
# that exact image, and WRONG before it.
.PHONY: ts-validated-repro ts-validated-repro-test
ts-validated-repro:
	@bash tools/ts_validated_repro.sh

ts-validated-repro-test:
	@bash tools/ts_validated_repro.sh --canary
