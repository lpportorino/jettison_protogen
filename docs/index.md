...
**Statistics:** 349 messages, 71 enums, 1157 fields
...
- [[proto/cmd.DayCamera.SceneAuto|SceneAuto]] — Sets the operator's day FULL-AUTO latch, reported back as `JonGuiDataScene.day_auto`. While the classifier's `shadow` flag is true the latch changes nothing else: the guest keeps reporting what it would select and never issues a mode change.
...
- [[proto/cmd.HeatCamera.SceneAuto|SceneAuto]] — Sets the operator's heat FULL-AUTO latch, reported back as `JonGuiDataScene.heat_auto`. While the classifier's `shadow` flag is true the latch changes nothing else.
...
- [[proto/ser.JonGuiDataHeatSceneClass|JonGuiDataHeatSceneClass]] — The scene classes the FULL-AUTO thermal classifier can hold or propose (`JonGuiDataScene.heat_class` / `heat_challenger`). The classifier needs a PRE-AGC thermal signal that no box publishes today, so every box reports `UNSPECIFIED` with reason `noinput`; the enum exists so the wire is ready for the day that telemetry lands.
...
- [[proto/ser.JonGuiDataSceneClass|JonGuiDataSceneClass]] — The scene classes the FULL-AUTO day classifier can hold or propose (`JonGuiDataScene.day_class` / `day_challenger`). Each maps onto one existing day FX mode in the guest (`mods/isp3a/scene/scene_classes.h`), which is the only place a class number is decided; a host test pins these values to that header.
