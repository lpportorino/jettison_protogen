 ...
 */
export interface SamTrackingHeat {
...
function createBaseSamTrackingHeat(): SamTrackingHeat {
  return {
    status: 0,
    state: 0,
    bboxX1: 0,
...
export const SamTrackingHeat: MessageFns<SamTrackingHeat> = {
  encode(message: SamTrackingHeat, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.status !== 0) {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): SamTrackingHeat {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseSamTrackingHeat();
  ...
  fromJSON(object: any): SamTrackingHeat {
    return {
  ...
  toJSON(message: SamTrackingHeat): unknown {
    const obj: any = {};
    if (message.status !== 0) {
  ...
  create<I extends Exact<DeepPartial<SamTrackingHeat>, I>>(base?: I): SamTrackingHeat {
    return SamTrackingHeat.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<SamTrackingHeat>, I>>(object: I): SamTrackingHeat {
    const message = createBaseSamTrackingHeat();
