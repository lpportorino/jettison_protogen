(ns protocol-gen.rust-access
  "The per-group ACCESS MODULE, in Rust, derived from the same projection the
   `.proto` text and the permission TRANSCRIPT are derived from.

   THE FLAT ARTEFACT IS THE TRANSCRIPT, and the word MIRROR is the nested
   tree's alone — `protocol-gen.mirror`'s docstring carries the split and why
   its own identifiers still spell the other word.

   WHY RUST AND NOT EDN. The transcript already carries direction, and it is
   EDN: a consumer reads it with a parser, at run time, and a typo in the key
   it looks under is a nil rather than a compile error. This module carries
   ...
   WHAT IT IS NOT. Like the transcript, it is a record of what the generator
   was told, not an enforcement mechanism. Nothing here stops a consumer
   sending a message it may not send.

   AND THE SCHEMA IS NOT THE MECHANISM EITHER. The reading to refuse is `what
   stops that is the group's schema not being able to express one` — the
   obvious one to reach for, and false. A schema constrains what an HONEST
   client CONSTRUCTS; bytes on the wire carry no trace of the generated code
   that produced them, so a peer that ignores its own schema, or was never
   ...
   `protocol-gen.mirror`'s docstring is this argument's home. Enforcement is a
   RECEIVE-side property and lives over the NESTED tree
   `protocol-gen.permission-tree` emits from this same projection. What this
   module adds is the direction the schema has nowhere to put, for a reviewer
   and for a consumer's own `if`.
...
(def banner
  ...
  (str "// GENERATED by protocol-gen — DO NOT EDIT.\n"
       ...
       "// `.proto` and the permission transcript beside it.\n"
       "//\n"
