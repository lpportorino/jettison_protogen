...
(defn cmd-builders-rust
  "Emit cmd_builders.rs: typed `build_set_value_command` (single double field),
   `build_action_command` (parameterless), and `build_set_enum_command` (single
   enum field), over every oneof-reachable cmd leaf."
  [db]
...
(defn generate-nodes
  ...
  (let [db (edn/read-string (slurp db-path))
    ...
    (let [{:keys [rust set-value-count action-count set-enum-count]} (cmd-builders-rust db)
          cf (io/file out-dir "cmd_builders.rs")]
      (spit cf rust)
      (t/log! :info ["Wrote" (.getPath cf) "with" set-value-count "set-value +"
                     action-count "action +" set-enum-count "set-enum command builder(s)"]))
